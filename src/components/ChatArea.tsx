...
const MentionText = ({ text }: { text: string }) => {
  ...
  while ((match = mentionRegex.exec(text)) !== null) {
    if (match.index > lastIndex) {
      parts.push(
        <span key={lastIndex} className="whitespace-pre-wrap break-words text-sm text-foreground/90">
          {text.slice(lastIndex, match.index)}
        </span>
      );
    }
    parts.push(
      <span key={match.index} className="rounded bg-primary/20 px-1 text-sm font-medium text-primary">
        {match[0]}
      </span>
    );
...
const MessageItem = ({ msg, onReply }: { msg: Message; onReply: (id: string) => void }) => {
  ...
  return (
    <div className="group relative flex gap-3 px-4 py-1 hover:bg-muted/30">
      <UserProfilePopup userId={msg.author_id} side="right">
        <Avatar className="mt-0.5 h-10 w-10 flex-shrink-0 cursor-pointer">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline gap-2">
          <UserProfilePopup userId={msg.author_id} side="right">
            <span className="cursor-pointer font-display text-sm font-semibold text-foreground hover:underline">{name}</span>
          </UserProfilePopup>
...
export const ChatArea = ({ channelId, channelName, serverId, showMembersToggle, showMembers, onToggleMembers }: ChatAreaProps) => {
